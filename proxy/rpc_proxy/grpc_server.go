...
func GrpcServer() {
	...
	proto2.RegisterEchoServer(server, &Servers{})
...
// token,timestamp,授权
func (s *Servers) UnaryEcho(ctx context.Context, req *proto2.EchoRequest) (*proto2.EchoResponse, error) {
...
func (s *Servers) ServerStreamingEcho(req *proto2.EchoRequest, stream proto2.Echo_ServerStreamingEchoServer) error {
	fmt.Println("----------ServerStreamingEcho-----------")

	// 服务端向客户单响应使用send函数
	for i := 0; i < 5; i++ { // 以流的形式发送多次
		err := stream.Send(&proto2.EchoResponse{Message: req.Message})
		if err != nil {
			return err
		}
	}
...
func (s *Servers) ClientStreamingEcho(stream proto2.Echo_ClientStreamingEchoServer) error {
	...
	for {
		...
		if err == io.EOF {
			...
			// 发送并关闭
			return stream.SendAndClose(&proto2.EchoResponse{Message: "Receive Finish"})
		} else if err != nil {
			log.Println(err)
			return err
		}
...
// 双向流式处理
func (s *Servers) BidirectionalStreamingEcho(stream proto2.Echo_BidirectionalStreamingEchoServer) error {
	...
	for {
		...
		if err == io.EOF {
			fmt.Println("消息发送完毕")
			stream.Send(&proto2.EchoResponse{Message: "Receive over"})
			return nil
		} else if err != nil {
		...
		// 回复响应
		err = stream.Send(&proto2.EchoResponse{Message: "Receive success!"})
		if err != nil {
